...
def extract_ocsp_url(cert_chain: List[str]) -> str:
    ...
    certificate = x509.load_pem_x509_certificate(str.encode(cert_chain[0]), default_backend())
...
def build_ocsp_request(cert_chain: List[str]) -> bytes:
    ...
    try:
        leaf_cert = x509.load_pem_x509_certificate(str.encode(cert_chain[0]), default_backend())
        issuer_cert = x509.load_pem_x509_certificate(str.encode(cert_chain[1]), default_backend())
...
def get_ocsp_response(ocsp_url: str, ocsp_request_data: bytes, proxy=None):
    ...
    try:
        ...
        if isinstance(err.reason, gaierror):
            raise Exception(f"{func_name}: {ocsp_url} is invalid or not known.") from None
...
def extract_ocsp_result(ocsp_response):
    ...
    try:
        ...
        return f"OCSP Status: {certificate_status[1]}", ocsp_response.next_update

    except ValueError as err:
        return f"{func_name}: {str(err)}", None
